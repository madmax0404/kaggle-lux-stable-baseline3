...
class EnvState:
    ...
    total_team_points: chex.Array
    """Total team points in the environment with shape (T) for T teams"""

    ### Rewards ###
    point_gain_rewards: chex.Array
    """Point gain rewards in the environment with shape (T) for T teams"""
    match_win_rewards: chex.Array
    """Team win rewards in the environment with shape (T) for T teams"""
    match_lose_rewards: chex.Array
    """Team lose rewards in the environment with shape (T) for T teams"""
    game_win_rewards: chex.Array
    """Game win rewards in the environment with shape (T) for T teams"""
    game_lose_rewards: chex.Array
    """Game lose rewards in the environment with shape (T) for T teams"""
    game_over_flag: bool = False
    game_over_flag_plus_one: bool = False
...
def gen_state(key: chex.PRNGKey, env_params: EnvParams, max_units: int, num_teams: int, map_type: int, map_width: int, map_height: int, max_energy_nodes: int, max_relic_nodes: int, relic_config_size: int) -> EnvState:
    ...
    state = EnvState(
        ...
        total_team_points=jnp.zeros(shape=(num_teams), dtype=jnp.int32),

        ### Rewards ###
        point_gain_rewards=jnp.zeros(shape=(num_teams), dtype=jnp.int32),
        match_win_rewards=jnp.zeros(shape=(num_teams), dtype=jnp.int32),
        match_lose_rewards=jnp.zeros(shape=(num_teams), dtype=jnp.int32),
        game_win_rewards=jnp.zeros(shape=(num_teams), dtype=jnp.int32),
        game_lose_rewards=jnp.zeros(shape=(num_teams), dtype=jnp.int32),
        game_over_flag=jnp.bool(False),
        game_over_flag_plus_one=jnp.bool(False),
