...
class Monitor(gym.Wrapper[ObsType, ActType, ObsType, ActType]):
    ...
    def __init__(
        ...
        self.rewards_0: list[float] = []
        self.rewards_1: list[float] = []
        self.needs_reset = True
        self.episode_returns_0: list[float] = []
        self.episode_returns_1: list[float] = []
        self.episode_lengths_0: list[int] = []
        self.episode_lengths_1: list[int] = []
    ...
    def step(self, action: ActType) -> tuple[ObsType, SupportsFloat, bool, bool, dict[str, Any]]:
        ...
        # print(reward)
        terminated = terminated["player_0"] + terminated["player_1"]
        truncated = truncated["player_0"] + truncated["player_1"]
        self.rewards_0.append(float(reward["player_0"]))
        self.rewards_1.append(float(reward["player_1"]))
        if terminated or truncated:
            self.needs_reset = True
            ep_rew_0 = sum(self.rewards_0)
            ep_rew_1 = sum(self.rewards_1)
            ep_len_0 = len(self.rewards_0)
            ep_len_1 = len(self.rewards_1)
            ep_info_0 = {"r": round(ep_rew_0, 6), "l": ep_len_0, "t": round(time.time() - self.t_start, 6)}
            ep_info_1 = {"r": round(ep_rew_1, 6), "l": ep_len_1, "t": round(time.time() - self.t_start, 6)}
            for key in self.info_keywords:
                ep_info_0[key] = info[key]
                ep_info_1[key] = info[key]
            self.episode_returns_0.append(ep_rew_0)
            self.episode_returns_1.append(ep_rew_1)
            self.episode_lengths_0.append(ep_len_0)
            self.episode_lengths_1.append(ep_len_1)
            self.episode_times.append(time.time() - self.t_start)
            ep_info_0.update(self.current_reset_info)
            ep_info_1.update(self.current_reset_info)
            if self.results_writer:
                self.results_writer.write_row(ep_info_0)
                self.results_writer.write_row(ep_info_1)
            info["episode_0"] = ep_info_0
            info["episode_1"] = ep_info_1
        self.total_steps += 1
