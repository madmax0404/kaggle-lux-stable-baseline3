...
class BaseAlgorithm(ABC):
    ...
    def _wrap_env(env: GymEnv, verbose: int = 0, monitor_wrapper: bool = True) -> VecEnv:
        ...
        if not isinstance(env, VecEnv):
            ...
            # NUM_ENVS = 8
            # env = SubprocVecEnv([lambda: env for _ in range(NUM_ENVS)])  # type: ignore[list-item, return-value]
    ...
    def _update_learning_rate(self, optimizers: Union[list[th.optim.Optimizer], th.optim.Optimizer]) -> None:
        ...
        self.logger.record("common_params/learning_rate", self.lr_schedule(self._current_progress_remaining))
        # print(optimizers)
        # self.logger.record("train/learning_rate", optimizers.param_groups[0]["lr"])
