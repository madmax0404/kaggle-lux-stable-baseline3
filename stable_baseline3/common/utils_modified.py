...
#     if isinstance(obs_player_0, np.ndarray):
#         return th.as_tensor(obs_player_0, device=device), th.as_tensor(obs_player_1, device=device)
#     elif isinstance(obs_player_0, dict):
#         return {key: th.as_tensor(_obs, device=device) for (key, _obs) in obs_player_0.items()}, {key: th.as_tensor(_obs, device=device) for (key, _obs) in obs_player_1.items()}
#     else:
#         raise Exception(f"Unrecognized type of observation {type(obs_player_0)}")
