...
class LuxAIS3Env(environment.Environment):
    ...
    def step_env(
        ...
        # print(action["player_0"])
        # print(action["player_1"])
        ...
        state = state.replace(total_team_points=state.total_team_points + team_scores)
        state = state.replace(point_gain_rewards=state.point_gain_rewards + team_scores)
        ...
        ### Reward Calculation
        match_started = state.match_steps == 0

        state = state.replace(
            match_win_rewards=jnp.where(
                match_ended, state.match_win_rewards.at[winner].add(1), state.match_win_rewards
            )
        )
        state = state.replace(
            match_win_rewards=jnp.where(
                match_started, jnp.zeros_like(state.match_win_rewards), state.match_win_rewards
            )
        )

        state = state.replace(
            match_lose_rewards=jnp.where(
                match_ended, state.match_lose_rewards.at[1 - winner].add(1), state.match_lose_rewards
            )
        )

        state = state.replace(
            match_lose_rewards=jnp.where(
                match_started, jnp.zeros_like(state.match_lose_rewards), state.match_lose_rewards
            )
        )

        game_winner_by_matchs = jnp.where(
            state.team_wins.max() == 3,
            jnp.argmax(state.team_wins),
            -1
        )

        game_ended = jnp.logical_and(
            game_winner_by_matchs != -1,
            state.game_over_flag == False
        )

        state = state.replace(
            game_over_flag_plus_one=jnp.where(
                state.game_over_flag, True, state.game_over_flag_plus_one
            )
        )

        state = state.replace(
            game_over_flag=jnp.where(
                game_ended, True, state.game_over_flag
            )
        )

        state = state.replace(
            game_win_rewards=jnp.where(
                game_ended, state.game_win_rewards.at[game_winner_by_matchs].add(1), state.game_win_rewards
            )
        )

        state = state.replace(
            game_win_rewards=jnp.where(
                match_started, jnp.zeros_like(state.game_win_rewards), state.game_win_rewards
            )
        )

        state = state.replace(
            game_lose_rewards=jnp.where(
                game_ended, state.game_lose_rewards.at[1 - game_winner_by_matchs].add(1), state.game_lose_rewards
            )
        )

        state = state.replace(
            game_lose_rewards=jnp.where(
                match_started, jnp.zeros_like(state.game_lose_rewards), state.game_lose_rewards
            )
        )
        ...
        for k in range(self.fixed_env_params.num_teams):
            # reward[f"player_{k}"] = jnp.where(state.point_gain_rewards[k] > 0, state.point_gain_rewards[k], -0.01)
            # reward[f"player_{k}"] = state.point_gain_rewards[k]
            reward[f"player_{k}"] = jnp.where(state.point_gain_rewards[k] > 0, state.point_gain_rewards[k] * 100, -0.1 * (state.match_steps / params.max_steps_in_match))
            reward[f"player_{k}"] += state.game_win_rewards[k] * 30000.0
            reward[f"player_{k}"] += state.game_lose_rewards[k] * -30000.0
            # reward[f"player_{k}"] += jnp.where(state.game_over_flag == False, state.match_win_rewards[k] * 5000.0, 0.0)
            # reward[f"player_{k}"] += jnp.where(state.game_over_flag == False, state.match_lose_rewards[k] * -2000.0, 0.0)

                                               
        
        state = state.replace(point_gain_rewards=jnp.zeros_like(state.point_gain_rewards))
    ...
    def get_obs(self, state: EnvState, params=None, key=None) -> EnvObs:
        ...
        for t in range(self.fixed_env_params.num_teams):
            ...
            team_obs = EnvObs(
                ...
                total_team_points=state.total_team_points,
                team_wins=state.team_wins,
                point_gain_rewards=state.point_gain_rewards,
                match_win_rewards=state.match_win_rewards,
                match_lose_rewards=state.match_lose_rewards,
                game_win_rewards=state.game_win_rewards,
                game_lose_rewards=state.game_lose_rewards,
                steps=state.steps,
                ...
                game_over_flag_plus_one=state.game_over_flag_plus_one
            )
